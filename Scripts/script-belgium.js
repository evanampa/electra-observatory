...
async function fetchData() {
  ...
  try {
    const response = await fetch("../db/belgium/belgium/spiders/belgium_data.json");
    ...
    const filteredData = data.filter((row) => row.seats !== null);
    const belgiumParties = Object.values(partiesData.belgium[0]);

    filteredData.forEach((row, index) => {
      const value = belgiumParties[index];
