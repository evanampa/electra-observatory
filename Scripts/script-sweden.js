...
async function fetchData() {
  ...
  try {
    const response = await fetch("../db/sweden/sweden/spiders/sweden-data.json");
    ...
    const filteredData = data.filter((row) => row.Seats !== "-");
    const swedenParties = Object.values(partiesData.sweden[0]);

    filteredData.sort((p1, p2) => {
      const votes1 = parseInt(p1.Seats); 
      const votes2 = parseInt(p2.Seats); 
      return votes2 - votes1; 
    });
    
    let txt = "";
    filteredData.forEach((value) => {
      txt += `${value.Party}: ${value.Seats}<br>`; 
    });

    filteredData.forEach((row, index) => {
      const value = swedenParties[index];
    ...
    filteredData.forEach((row) => {
      ...
      percentBar.innerHTML = `
        <div class="name">
          ...
          <div class="percentage">${row.Votes}</div>
        </div>
        <div class="bar">
